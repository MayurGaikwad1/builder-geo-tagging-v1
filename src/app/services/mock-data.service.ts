...
export class MockDataService {
  ...
  private generateMockLocationHistory() {
    ...
    const locations = [
      { name: "Main Branch Office", lat: 40.7128, lng: -74.006 },
      {
        name: "Regional Office, Business District",
        lat: 40.7505,
        lng: -73.9934,
      },
      { name: "Customer Office, Tech Park", lat: 40.7589, lng: -73.9851 },
      {
        name: "Partner Office, Financial District",
        lat: 40.7074,
        lng: -74.0113,
      },
      ...
      for (let j = 0; j < dailyCount; j++) {
        ...
        const mockLocation: LocationData = {
          ...
          reportingManagerName:
            employeeData.reportingManagerName || "Jane Smith",
          dateTime: time,
        ...
        branchCheckIn: {
          dateTime: isWorkDay
            ? new Date(
                ...
                date.getMonth(),
                date.getDate(),
                isOnTime ? 9 : 9 + Math.floor(Math.random() * 2),
        ...
        if (isCompleted) {
          ...
          if (!isOnTime) {
            startDateTime.setMinutes(
              startDateTime.getMinutes() + Math.floor(Math.random() * 30),
            ); // Up to 30 min late
          }
        ...
        const meeting: Meeting = {
          ...
          checkInStatus: isCompleted
            ? isOnTime
              ...
              : "after"
            : "before",
          meetingCode: !partner.agentCode
            ? Math.floor(100000 + Math.random() * 900000).toString()
            : undefined,
          status: isFutureMeeting
            ? "scheduled"
            : isCompleted
              ? "completed"
              : "scheduled",
