...
@Component({
  ...
  template: `
    <div class="fixed top-4 right-4 z-50 space-y-4" style="max-width: 400px;">
      <div 
        *ngFor="let notification of notificationService.activeNotifications(); trackBy: trackById"
        class="notification-container transform transition-all duration-300 ease-in-out"
        [class]="getNotificationClasses(notification)"
      >
        <div class="flex items-start">
          <div class="flex-shrink-0">
            <svg class="w-5 h-5 mt-0.5" [innerHTML]="getNotificationIcon(notification.type)"></svg>
          </div>
          <div class="ml-3 flex-1">
            <h4 class="text-sm font-semibold">{{ notification.title }}</h4>
            <p class="text-sm mt-1 whitespace-pre-line">{{ notification.message }}</p>
          </div>
